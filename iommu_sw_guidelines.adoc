...
... When a `PQ` overflow condition occurs, software may observe incomplete
    page-request groups and the IOMMU might have automatically responded (see
    <<ATS_PRI>> to a "Page Request" in such groups if the "Last Request in PRG"
    flag was set to 1. Software should ignore the such incomplete groups and not
    service such groups.
