...
. Stop and report failure if big-endian memory access is needed and the 
  `capabilities.END` field is 0 (i.e. little-endian only).
. Stop and report failure if wired-interrupts are needed for IOMMU initiated
  interrupts and `capabilities.IGS` is not `WIS` and is not `BOTH`.
...
. If big-endian memory access is required then set `fctrl.END` to 1 if 
  the field is not already 1
. If wired interrupts from IOMMU are needed then set `fctrl.WIS` to 1 if 
  the field is not already 1
...
. If the IOMMU is configured to use MSI, then each vector `V` is an index into
  ...
  an interrupt identity `D`. Configure the `msi_addr_V` with value `A`, 
  `msi_data_V` with value `D`. Configure the interrupt mask `M` in 
  `msi_vec_ctrl_V` appropriately.
...
. To program the fault queue, first determine the number of entries N needed in
  ...
  of two.  Let `N = 2^k^`. If `N` is 64 or lower then allocate a memory buffer 
  that is aligned to 4-KiB page address and is of size `N` x 16-bytes. If `N` 
  is greater than 64 then allocate a memory buffer that is naturally aligned to
  a `N` * 64-byte address boundary. Let the physical page number of the buffer 
...
. To program the DDT pointer, first determine the supported `device_id` width `D` 
  ...
  allocated memory to all 0. Determine the mode `M` of the DDT based on `D` and 
  if the IOMMU requires base or extended format device-contexts.
. If extended format device-context are used then
.. If `D` is less than or equal to 6-bits then `M = 1LVL`
.. If `D` is less than or equal to 15-bits then `M = 2LVL`
.. If `D` is less than or equal to 24-bits then `M = 3LVL`
. If base format device-context are used then
.. If `D` is less than or equal to 7-bits then `M = 1LVL`
.. If `D` is less than or equal to 16-bits then `M = 2LVL`
.. If `D` is less than or equal to 24-bits then `M = 3LVL`
