...
==== Changing VS-stage page table entry

A `DC` may be configured with a VS-stage page table (when `DC.tc.PDTV=0`) or
a directory of VS-stage page tables selected using `process_id` from a
process-directory-table (when `DC.tc.PDTV=1`).

When a change is made to a VS-stage page table, and the associated G-stage is
...
Between a change to the VS-stage PTE and when an invalidation command to
invalidate the cached PTE is processed by the IOMMU, the IOMMU may use the
old PTE value or the new PTE value.
...
the A and/or D bit in the VS-stage and/or G-stage PTEs then software must
...
When software upgrades a page in a VS-stage PTE and/or a G-stage PTE to
...
When VS-stage and/or G-stage page tables are modified, invalidations may be
...
. If the device-context for the device is already valid then first mark the
  ...
  cache G/VS-stage page table entries, DDT entries, MSI PT entries
...
. Set `EN_ATS` and/or `EN_PRI` to 0 in the device-context. If `EN_ATS` is set to
  ...
  to 0. 
. Queue commands to the IOMMU to invalidate all cached G/VS-stage page table
