...
This section provides guidelines to software developers on correct andg
...
* Registers that are 64-bit wide may be accessed using either a 32-bit org
  a 64-bit access.
...
. Stop and report failure if big-endian memory access is needed and theg
...
. If wired-signaled-interrupts are needed for IOMMU initiated interrupts and
  ...
  already 1.
. Stop and report failure if other required capabilitiesg
  (e.g. virtual-addressing modes, MSI translation, etc.) are not supported.
. The `icvec` register is used to program an interrupt vector for eachg
  interrupt cause. Determine the number of vectors supported by the IOMMU byg
  writing 0xF to each field and reading back the number of writable bits. Ifg
  the number of writable bits is `N` then the number of supported vectors is
  `2^N^`. For each cause `C` associate a vector `V` with the cause. `V` is ag
  number between 0 and `(2^N^ - 1)`.g
. If the IOMMU is configured to use wired interrupts, then each vector `V`g
  corresponds to an interrupt wire connected to a platform level interruptg
  controller (e.g. APLIC). Determine the interrupt controller configurationg
  register to be programmed for each such wire using configuration information
  provided by configuration mechanisms such as device tree and program theg
  interrupt controller.
. If the IOMMU is configured to use MSI, then each vector `V` is an index into
  the `msi_cfg_tbl`. For each vector `V`, allocate a MSI address `A` andg
  an interrupt identity `D`. Configure the `msi_addr_V` register with value `A`,
  `msi_data_V` register with value `D`. Configure the interrupt mask `M` ing
  `msi_vec_ctl_V` register appropriately.
. To program the command queue, first determine the number of entries `N` needed
  in the command queue. The number of entries in the command queue is always ag
...
. To program the fault queue, first determine the number of entries N needed in
  the fault queue. The number of entries in the fault queue is always a powerg
...
. To program the page-request queue, first determine the number of entries `N`g
...
. To program the DDT pointer, first determine the supported `device_id` width `Dw`g
  and the format of the device-context data structure. If `capabilities.MSI` is
  0, then the IOMMU uses base-format device-contexts else extended-formatg
...
send to the IOMMU through the `CQ` when modifying the IOMMU in-memory datag
...
A `IOFENCE.C` command may be used by software to ensure that all previousg
...
* else
** If `DC.tc.PDTV==1 || DC.tc.PDTV == 0 && DC.fsc.MODE == Bare`g
...
* If `DC.iohgatp.MODE != Bare`
** `IOTINVAL.VMA` with `GV=1`, `AV=0`, `PV=1`, `GSCID=DC.iohgatp.GSCID`,g
   and `PSCID=PC.PSCID`
* else
...
number `I` that corresponds to an untranslated MSI address `A` then followingg
invalidations must be performed:

* If `DC.iohgatp.MODE == Bare`
** If `DC.fsc.iosatp.MODE == Bare`, then `IOTINVAL.VMA` with `GV=0`, `PSCV=0`,g
   `AV=1`, and `ADDR[63:12]=A[63:12]`
** If `DC.fsc.iosatp.MODE != Bare`, then `IOTINVAL.VMA` with `GV=0`, `PSCV=1`,g
   `AV=1`, `PSCID=DC.ta.PSCID`, and `ADDR[63:12]=A[63:12]`
* else
** `IOTINVAL.GVMA` with `GV=AV=1`, `ADDR[63:12]=A[63:12]` andg
...
If software changes a non-leaf G-stage page-table entry of a VMg
...
selected using `process_id` from a process-directory-table (wheng
`DC.tc.PDTV=1`).
...
appropriate for the modification as specified in <<IVMA>>. g
...
When software upgrades a page in S/VS-stage PTE and/or a G-stage PTE tog
...
IOMMU to cache multiple entries that match a single address. The IOMMU mayg
use either the old non-leaf PTE or the new non-leaf PTE but the behavior is
otherwise well defined.

When promoting and/or demoting page sizes, software must ensure that theg
...
promotion or demotion. The behavior of the IOMMU if other attributes areg
...
following the rules defined in PCIe ATS specifications cite:[PCI].g

If software generates ATS invalidate requests at a rate that exceeds theg
average DevATC service rate then flow control mechanisms may be triggered byg
the device to throttle the rate and a side effect of this is congestion
spreading to other channels and links and could lead to performanceg
degradation. An ATS capable device publishes the maximum number ofg
...
Software must limit the number of outstanding ATS invalidations queued tog
the device advertised limit.

The `RID` field is used to specify the routing ID of the ATS invalidationg
...
When ATS protocol is enabled for a device, the IOMMU may still cacheg
...
This specification does not allow the caching of S/VS/G-stage PTEs whose `V`g
(valid) bit is clear, non-leaf DDT entries whose `V` (valid) bit is clear,g
...
IOMMU may generate an from the `CQ`, the `FQ`, the `PQ`, or the HPM. Eachg
...
. If `ipsr.cip` bit is set then an interrupt is pending from the `CQ`.g
.. Read the `cqcsr` register.
.. Determine if an error caused the interrupt and if so, the cause of theg
   error by examining the state of the `cmd_to`, `cmd_ill`, and `cqmf` bitsg
...
.. If errors have occurred, correct the cause of the error and clear the bits
   corresponding to the corrected errors in `cqcsr` by writing 1 to the bits.g
... Clearing all error indication bits in `cqcsr` re-enables command processing.g
.. An IOMMU that supports wired-interrupts may be requested to generate ang
   interrupt from the command queue on completion of a `IOFENCE.C` command.g
   This cause is indicated by the `fence_w_ip` bit. Note that commandg
...
. If `ipsr.fip` bit is set then an interrupt is pending from the `FQ`.g
...
.. Determine if an error caused the interrupt and if so, the cause of the error
   ...
   are set then the `FQ` encountered an error and fault/event reporting isg
   temporarily disabled.
.. If errors have occurred, correct the cause of the error and clear the bits
   corresponding to the corrected errors in `fqcsr` by writing 1 to the bits.g
... Clearing all error indication bits in `cqcsr` re-enables fault/eventg
   reporting.g
...
. If `ipsr.pip` bit is set then an interrupt is pending from the `PQ`.g
.. Read the `pqcsr`register.
.. Determine if an error caused the interrupt and if so, the cause of the error
   by examining the state of the `pqmf` and `pqof` bits. If either of these bits
   are set then the `PQ` encountered an error and "Page Request" reporting isg
   temporarily disabled.
.. If errors have occurred, correct the cause of the error and clear the bits
   corresponding to the corrected errors in `pqcsr` by writing 1 to the bits.g
... Clearing all error indication bits in `pqcsr` re-enables "Page Request"g
   reporting.
...
. Place the device in an idle state such that no transactions are generatedg
  by the device.g
. If the device-context for the device is already valid then first mark theg
  device-context as invalid and queue commands to the IOMMU to invalidate allg
  cache G/S/VS-stage page table entries, DDT entries, MSI PT entriesg
...
. Mark the device-context as valid.g
...
. Queue commands to the IOMMU to invalidate all cached G/S/VS-stage page tableg
  entries, DDT entries, MSI PT entries (if required), and PDT entriesg
  (if required).
