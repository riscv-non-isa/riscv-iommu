...
... When a `PQ` overflow condition occurs, software may observe incomplete
    page-request groups due to the "Page Request" messages received being
    dropped due to the overflow. The IOMMU might have automatically responded (see
    <<ATS_PRI>>) to a droped "Page Request" in such groups if the "Last Request in
    PRG" flag was set to 1. Software should ignore and not service the such
    incomplete groups.
