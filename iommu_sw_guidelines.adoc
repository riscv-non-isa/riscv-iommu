...
. Stop and report failure if big-endian memory access is needed and the 
  `capabilities.BE` field is 0 (i.e. only one endianness) and `fctl.SBE` is
  0 (i.e. little endian).
. If big-endian memory access is needed and the `capabilities.BE` field is 1
  (i.e. both endianness supported), set `fctl.SBE` to 1 (i.e. big endian)
