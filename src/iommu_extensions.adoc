...
The IOMMU `capabilities` register (xref:iommu_registers.adoc#CAP[IOMMU capabilities]) is extended with a `QOSID` field
...
extended to add a register named `iommu_qosid` (xref:iommu_registers.adoc#IOQOSID[IOMMU QoS ID]). This register is
used to configure the Quality of Service (QoS) IDs associated with
IOMMU-originated requests. The `ta` field of the device context (xref:iommu_data_structures.adoc#DC_TA[Translation attributes]) is
...
used to encode the translation range size in `tr_response` (xref:iommu_registers.adoc#TRR_RSP[Translation-response])
register.
