...
| xref:iommu_extensions.adoc#QOSID[*Quality-of-Service (QoS) Identifiers Extension*]
                                                             |*1.0*
                                                                      |*Ratified*
| xref:iommu_extensions.adoc#NLINV[*Non-leaf PTE Invalidation Extension*]
                                                             |*1.0*
                                                                      |*Ratified*
| xref:iommu_extensions.adoc#ARINV[*Address Range Invalidation Extension*]
                                                             |*1.0*
                                                                      |*Ratified*
| xref:iommu_extensions.adoc#SVRSW[*PTE Reserved-for-Software Bits 60-59*]
                                                             |*1.0*
                                                                      |*Ratified*
|===
...
The IOMMU `capabilities` register (xref:iommu_registers.adoc#CAP[]) is extended with a `QOSID` field
...
extended to add a register named `iommu_qosid` (xref:iommu_registers.adoc#IOQOSID[]). This register is
used to configure the Quality of Service (QoS) IDs associated with
IOMMU-originated requests. The `ta` field of the device context (xref:iommu_data_structures.adoc#DC_TA[]) is
...
used to encode the translation range size in `tr_response` (xref:iommu_registers.adoc#TRR_RSP[])
register.
