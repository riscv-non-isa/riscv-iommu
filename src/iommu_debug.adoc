...
translate an IOVA (xref:iommu_data_structures.adoc#P2IOVA[]) as an Untranslated Request. The result of the
...
that of a virtual interrupt file (xref:iommu_data_structures.adoc#MSI_ID[]) and the corresponding MSI PTE is
...
* `tr_req_iova` (xref:iommu_registers.adoc#TRR_IOVA[])
* `tr_req_ctl` (xref:iommu_registers.adoc#TRR_CTRL[])

The translation-response interface consists of a single 64-bit RO register
`tr_response` (xref:iommu_registers.adoc#TRR_RSP[])
