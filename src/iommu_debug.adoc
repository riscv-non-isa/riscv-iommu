...
that of a virtual interrupt file (xref:iommu_data_structures.adoc#MSI_ID[MSI address mask and pattern]) and the corresponding MSI PTE is
...
* `tr_req_iova` (xref:iommu_registers.adoc#TRR_IOVA[Translation-request IOVA])
* `tr_req_ctl` (xref:iommu_registers.adoc#TRR_CTRL[Translation-request control])

The translation-response interface consists of a single 64-bit RO register
`tr_response` (xref:iommu_registers.adoc#TRR_RSP[Translation-response])
