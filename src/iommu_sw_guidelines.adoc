...
specified in xref:iommu_in_memory_queues.adoc#IVMA[].
...
the modification as specified in xref:iommu_in_memory_queues.adoc#IVMA[].
...
... When a `PQ` overflow condition occurs, software may observe incomplete
  ...
  IOMMU might have automatically responded (see xref:iommu_data_structures.adoc#ATS_PRI[]) to a dropped
