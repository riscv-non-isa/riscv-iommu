...
specified in xref:iommu_in_memory_queues.adoc#IVMA[IOTINVAL.VMA].
...
the modification as specified in xref:iommu_in_memory_queues.adoc#IVMA[IOTINVAL.VMA].
...
... When a `PQ` overflow condition occurs, software may observe incomplete
    ...
    IOMMU might have automatically responded (see xref:iommu_data_structures.adoc#ATS_PRI[PCIe ATS Page Request handling]) to a dropped
