...
* *Base Format* - is 32-bytes in size used when the special treatment of MSI
  as specified in xref:iommu_data_structures.adoc#MSI_TRANS[] is not supported by the IOMMU.

* *Extended Format* - is 64-bytes in size and extends the base format `DC` with
  additional fields to translate MSIs as specified in xref:iommu_data_structures.adoc#MSI_TRANS[].
...
determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[]). The IOMMU may read the `DC` fields in any
order.
...
faults that are not reported when `DTF` is 1 are listed in xref:iommu_in_memory_queues.adoc#FAULT_CAUSE[].
...
in xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-0[] and xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-1[]. If `fctl.GXL` is 1 then the
...
schemes for guest physical addresses as defined in xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC-0[] and
xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC-1[].
...
as defined in xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-0[] and xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-1[]. The `iosatp.PPN`
...
. `DC.tc.PDTV` is 0 and `DC.fsc.iosatp.MODE` encoding is not a valid
  encoding as determined by xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-0[] and
  xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-1[].
...
. `DC.iohgatp.MODE` encoding is not a valid encoding as determined
  by xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC-0[] and xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC-1[].
...
scheme and its encodings are as defined in xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-0[] and
xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-1[]. The `DC.tc.SXL` field controls the supported paged
...
. `PC.fsc.MODE` encoding is not valid as determined by xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-0[] and
  xref:iommu_data_structures.adoc#IOSATP_MODE_ENC-1[].
...
(xref:iommu_data_structures.adoc#CACHING[]) are implemented, the translation process may use the `GSCID` and
...
. Use `device_id` to then locate the device-context (`DC`) as specified in
  xref:iommu_data_structures.adoc#GET_DC[].
...
. Locate the process-context (`PC`) as specified in xref:iommu_data_structures.adoc#GET_PC[].
. if any of the following conditions hold then stop and report
  "Transaction type disallowed" (cause = 260).
...
. If MSI address translations using MSI page tables is enabled
  ...
  specified in xref:iommu_data_structures.adoc#MSI_TRANS[] is invoked. If the GPA `A` is not determined to be
...
provided in xref:iommu_hw_guidelines.adoc#IOBR_FAULT_RESP[]. The fault may be reported using the fault/event
reporting mechanism and fault record formats specified in xref:iommu_in_memory_queues.adoc#FAULT_QUEUE[].
...
specified in xref:iommu_data_structures.adoc#ATS_FAULTS[].
...
. If the `DC` is misconfigured as determined by rules outlined in
  xref:iommu_data_structures.adoc#DC_MISCONFIG[] then stop and report "DDT entry misconfigured" (cause = 259).
...
. If the `PC` is misconfigured as determined by rules outlined in
  xref:iommu_data_structures.adoc#PC_MISCONFIG[] then stop and report "PDT entry misconfigured" (cause = 267).
...
When MSI address translation is supported (`capabilities.MSI_FLAT`, xref:iommu_registers.adoc#CAP[]),
...
. Let `DC` be the device-context located using the `device_id` of the device
  using the process outlined in xref:iommu_data_structures.adoc#GET_DC[].
. Determine if the address `A` is an access to a virtual interrupt file as
  specified in xref:iommu_data_structures.adoc#MSI_ID[].
...
determined to be an MSI address using the rules defined by the xref:iommu_data_structures.adoc#MSI_ID[], but
...
through 5 of xref:iommu_data_structures.adoc#P2IOVA[]--to determine if ATS and PRI are enabled for the
...
page-request-queue (`PQ`) (See xref:iommu_in_memory_queues.adoc#PRQ[]). Following suitable processing of the
...
conditions are specified in xref:iommu_in_memory_queues.adoc#PRQ[].
...
|Device Directory Table |`device_id`                | xref:iommu_in_memory_queues.adoc#IDDT[IODIR.INVAL_DDT]
|Process Directory Table|`device_id`, `process_id`  | xref:iommu_in_memory_queues.adoc#IPDT[IODIR.INVAL_PDT]
|First-stage page table
 (when second-stage is
  not Bare)             |`GSCID`, `PSCID`, and IOVA | xref:iommu_in_memory_queues.adoc#IVMA[IOTINVAL.VMA]
|First-stage page table
 (when second-stage is
  Bare)                 |`PSCID`, and IOVA          | xref:iommu_in_memory_queues.adoc#IVMA[IOTINVAL.VMA]
|Second-stage page table|`GSCID`, `GPA`             | xref:iommu_in_memory_queues.adoc#IGVMA[IOTINVAL.GVMA]
|MSI page table         |`GSCID`, `GPA`             | xref:iommu_in_memory_queues.adoc#IGVMA[IOTINVAL.GVMA]
|===
...
* If the update to a field will make the field inconsistent with another field
  ...
  outlined in xref:iommu_data_structures.adoc#CACHING[] to invalidate any previous copies of that entry that
...
* The IOMMU is not required to immediately observe the software update to an
  entry. Software must use the commands outlined in xref:iommu_data_structures.adoc#CACHING[] to invalidate
...
software uses the commands outlined in xref:iommu_data_structures.adoc#CACHING[] to invalidate any previous
