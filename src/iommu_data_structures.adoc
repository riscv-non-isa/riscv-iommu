...
determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[Features-control register]). The IOMMU may read the `DC` fields in any
order.
...
faults that are not reported when `DTF` is 1 are listed in xref:iommu_in_memory_queues.adoc#FAULT_CAUSE[Fault record CAUSE field encodings].
...
provided in xref:iommu_hw_guidelines.adoc#IOBR_FAULT_RESP[Aborting transactions]. The fault may be reported using the fault/event
reporting mechanism and fault record formats specified in xref:iommu_in_memory_queues.adoc#FAULT_QUEUE[Fault/Event-Queue].
...
When MSI address translation is supported (`capabilities.MSI_FLAT`, xref:iommu_registers.adoc#CAP[IOMMU capabilities]),
...
page-request-queue (`PQ`) (See xref:iommu_in_memory_queues.adoc#PRQ[Page-Request-Queue]). Following suitable processing of the
...
conditions are specified in xref:iommu_in_memory_queues.adoc#PRQ[Page-Request-Queue].
