...
When second-stage address translation is not Bare, the `DC/PC` holds the PPN of the
...
When `DC.tc.GIPC = 1`, the `PPN` field holds a supervisor PPN. The supervisor physical
...
process, as controlled by the `DC/PC.iohgatp`, into a supervisor physical address.
