...
in <<IOSATP_MODE_ENC-0>> and <<IOSATP_MODE_ENC-1>>. If `fctl.GXL` is 1 then the
...
schemes for guest physical addresses as defined in <<IOHGATP_MODE_ENC-0>> and
<<IOHGATP_MODE_ENC-1>>.
...
as defined in <<IOSATP_MODE_ENC-0>> and <<IOSATP_MODE_ENC-1>>. The `iosatp.PPN`
...
. `DC.tc.PDTV` is 0 and `DC.fsc.iosatp.MODE` encoding is not a valid
   encoding as determined by <<IOSATP_MODE_ENC-0>> and
   <<IOSATP_MODE_ENC-1>>.
...
. `DC.iohgatp.MODE` encoding is not a valid encoding as determined
  by <<IOHGATP_MODE_ENC-0>> and <<IOHGATP_MODE_ENC-1>>.
...
scheme and its encodings are as defined in <<IOSATP_MODE_ENC-0>> and
<<IOSATP_MODE_ENC-1>>. The `DC.tc.SXL` field controls the supported paged
...
. `PC.fsc.MODE` encoding is not valid as determined by <<IOSATP_MODE_ENC-0>> and
  <<IOSATP_MODE_ENC-1>>.
...
through 5 of <<P2IOVA>>--to determine if ATS and PRI are enabled for the
...
[NOTE]
====
A "Stop Marker" is encoded as a "Page Request" with the L, W, and R fields set
to 1, 0, and 0 respectively. Per PCIe, the behavior is undefined if the "Stop
Marker" has no PASID.
====
