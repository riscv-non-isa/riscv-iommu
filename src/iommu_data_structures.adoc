...
`iohgatp.mode` is not `Bare` and `GIPC` is disabled, `pdtp.PPN` as well as `pdte.PPN` are Guest
...
When `iohgatp.mode` is not `Bare` and `GIPC` is enabled, `pdtp.PPN` as well
...
. If `iohgatp.mode != Bare` and `GIPC` is disabled, then `a` is a GPA. Invoke the process
  ...
  subsequent steps. If `iohgatp.mode == Bare` or `GIPC` is enabled, then `a` is a SPA.
. If `i == 0` go to step 9.
...
. Let `PC` be the value of the 16/32 bytes at address `a + PDI[0] x 16/32`. If accessing `PC`
