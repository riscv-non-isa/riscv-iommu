...
determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[Features-control register]).
...
extension xref:iommu_extensions.adoc#NLINV[Non-leaf PTE Invalidation Extension]. The definition of the `S` bit is provided by the address
range invalidation extension xref:iommu_extensions.adoc#ARINV[Address Range Invalidation Extension].
...
specified to have a timeout, then the `cmd_to` bit in `cqcsr` xref:iommu_registers.adoc#CSR[Command-queue CSR] is set to
...
from the command queue to be generated (by setting `cqcsr.fence_w_ip` - xref:iommu_registers.adoc#CSR[Command-queue CSR])
...
write encounters a memory fault, the `cmd_mf` bit in `cqcsr` xref:iommu_registers.adoc#CSR[Command-queue CSR] is set to
...
supported by the IOMMU (see xref:iommu_registers.adoc#CAP[IOMMU capabilities]).
...
as determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[Features-control register]).
...
Response” message as specified in xref:iommu_data_structures.adoc#ATS_PRI[PCIe ATS Page Request handling].
...
endianness as determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[Features-control register]).
