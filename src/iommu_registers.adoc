...
After a reset the caches (xref:iommu_data_structures.adoc#CACHING[Caching in-memory data structures]) must have no valid entries.
...
|0     |`BE`       |WARL      | When 0, IOMMU accesses to memory resident data
                                structures, as specified in xref:iommu_data_structures.adoc#ENDIAN_CONFIG[Endianness of memory access to data structures],
...
|2     |`GXL`      |WARL      | Controls the address-translation schemes that
                                ...
                                defined in xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC[Encodings of iohgatp.MODE].
...
(xref:iommu_hw_guidelines.adoc#IOBR_FAULT_RESP[Aborting transactions]). It also ensures that all transactions and previous
...
|0     |`pqen`   |RW        | The page-request-enable bit enables the
                              ...
                              xref:iommu_data_structures.adoc#ATS_PRI[PCIe ATS Page Request handling].
