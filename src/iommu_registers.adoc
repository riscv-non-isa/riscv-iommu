...
|0     |`capabilities`  |8   |<<CAP, Capabilities of the
                                     IOMMU>>              | No
|8     |`fctl`          |4   |xref:iommu_registers.adoc#FCTRL[Features control] | No
|12    |custom          |4   |Designated For custom use   |
|16    |`ddtp`          |8   |<<DDTP, Device directory
                              table pointer>>             | No
|24    |`cqb`           |8   |xref:iommu_registers.adoc#CQB[Command-queue base] | No
|32    |`cqh`           |4   |xref:iommu_registers.adoc#CQH[Command-queue head] | No
|36    |`cqt`           |4   |xref:iommu_registers.adoc#CQT[Command-queue tail] | No
|40    |`fqb`           |8   |xref:iommu_registers.adoc#FQB[Fault-queue base]   | No
|48    |`fqh`           |4   |xref:iommu_registers.adoc#FQH[Fault-queue head]   | No
|52    |`fqt`           |4   |xref:iommu_registers.adoc#FQT[Fault-queue tail]   | No
...
|72    |`cqcsr`         |4   |xref:iommu_registers.adoc#CSR[Command-queue CSR]  | No
|76    |`fqcsr`         |4   |xref:iommu_registers.adoc#FQCSR[Fault-queue CSR]  | No
...
|96    |`iohpmcycles`   |8   |xref:iommu_registers.adoc#CYC[HPM cycles counter] | if `capabilities.HPM==0`
|104   |`iohpmctr1-31`  |248 |xref:iommu_registers.adoc#CTR[HPM event counters] | if `capabilities.HPM==0`
|352   |`iohpmevt1-31`  |248 |xref:iommu_registers.adoc#EVT[HPM event selector] | if `capabilities.HPM==0`
...
|624   |`iommu_qosid`   |4   |xref:iommu_registers.adoc#IOQOSID[IOMMU QoS ID]   | if `capabilities.QOSID==0`
...
After a reset the caches (xref:iommu_data_structures.adoc#CACHING[]) must have no valid entries.
...
|0     |`BE`       |WARL      | When 0, IOMMU accesses to memory resident data
                                structures, as specified in xref:iommu_data_structures.adoc#ENDIAN_CONFIG[],
...
|2     |`GXL`      |WARL      | Controls the address-translation schemes that
                                ...
                                defined in xref:iommu_data_structures.adoc#IOHGATP_MODE_ENC-0[] and
...
(xref:iommu_hw_guidelines.adoc#IOBR_FAULT_RESP[]). It also ensures that all transactions and previous
...
|0     |`pqen`   |RW        | The page-request-enable bit enables the
                              ...
                              xref:iommu_data_structures.adoc#ATS_PRI[].
...
If the conditions to set that bit are still present (See xref:iommu_registers.adoc#IPSR_FIELD[]) or if
...
|14:0   |`eventID`  |WARL     a| Indicates the event to count. A value of 0
                                 ...
                                 events defined in the xref:iommu_registers.adoc#Event_list[]. +
