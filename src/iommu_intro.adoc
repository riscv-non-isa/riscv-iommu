...
in xref:iommu_intro.adoc#fig:device-isolation[] the OS may configure the IOMMU with a page table to
...
xref:iommu_intro.adoc#fig:dma-translation-direct-device-assignment[] illustrates the concept.
...
(see xref:iommu_intro.adoc#MSI_REDIR[]) or to a memory-resident interrupt file. The IOMMU is
...
xref:iommu_intro.adoc#fig:iommu-for-guest-os[] illustrates the concept.
...
xref:iommu_intro.adoc#fig:example-soc-with-iommu[] shows an example of a typical system on a chip
...
and has several interfaces (see xref:iommu_intro.adoc#fig:iommu-interfaces[]):
...
register xref:iommu_registers.adoc#CAP[].
