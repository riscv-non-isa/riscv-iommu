...
Using the same VS-stage and G-stage page table formats in IOMMU for address
...
the same G and VS-stage tables to be used by both MMU and the IOMMU.
...
the required address translation and protection. The G-stage for such devices
may be placed in `Bare` mode.
...
<<fig:dma-translation-direct-device-assignment>> diagram illustrates the concept. 
The device D1 is directly
...
<<fig:iommu-for-guest-os>> diagram illustrates the concept. The IOMMU is configured
to perform address translation using a VS-stage and G-stage page table for device D1.
For device D2 the VS-stage is `Bare` and only the G-stage performs the address
translations. The host OS or hypervisor may also retain a device, such as D3, for its
own use. For device D3, the G-stage is configured to be in `Bare` mode.
