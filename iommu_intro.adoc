...
Devices perform DMA using an I/O virtual address (IOVA). The IOVA may be a
supervisor physical address (SPA), guest physical address (GPA), or a virtual
address (VA). The IOMMU employs a two-stage address translation process to
translate the IOVA to an SPA and to enforce memory protections for the DMA.
The first stage (VS-stage) is used to convert an IOVA into a GPA. The GPA is
then converted in the second stage (G-stage) into a SPA. 
...
If the scheme for the VS-stage is configured as `Bare` then the IOVA is equal to
a GPA and there is no further VS-stage memory protection. If the scheme for the
G-stage is configured as `Bare` then the GPA are equal to SPA and there is no
further G-stage memory protection.

The IOMMU uses same page table formats as used by the CPU's MMU for VS-stage and
G-stage address translation. Using the same pagee table formats as the CPU’s MMU
removes some complexity from the core kernel memory management for DMA. Use of
an identical format also allows the same G-stage and VS-stage tables to be used
by both the CPU MMU and the IOMMU.
...
In the absence of an IOMMU a DMA could be used to access any memory, such as
privileged memory, and cause malicious or unintended corruptions. This may be
due to hardware bugs, device driver bugs, or by malicious software/hardware.
...
As depicted in <<fig:device-isolation>> the Operating System may configure the
IOMMU with a page table to translate the IOVA and thereby limit the addresses
that may be accessed to those allowed by the page table.

Legacy 32-bit devices cannot access the memory above 4 GiB. The IOMMU, through
its address remapping capability, offers a simple mechanism for the DMA to
...
The IOMMU can be useful as it permits to allocate large regions of memory for
I/O without the need for all of the memory to be contiguous. A contiguous
virtual address range can map to such fragmented physical addresses and the
device programmed with the virtual address range.
...
When the IOMMU is used by a non-virtualized OS, the VS-stage suffices to provide
the required address translation and protection function and the G-stage may be
disabled.
...
to VM-2. The VMM configures the G-stage page table to be used for each device
...
those discussed for a non-virtualized OS through the use of a VS-stage page
table that it controls.

With two-stage address translations enabled, the IOVA is first translated to
...
<<fig:iommu-for-guest-os>> illustrates the concept. The IOMMU is configured
to perform address translation using a VS-stage and G-stage page table for
device D1. For device D2 the VS-stage is `Bare` and only the G-stage is
enabled. The host OS or hypervisor may also retain a device, such as D3, for
its own use. For device D3, the G-stage is `Bare`.
...
.. The device and process directories to get the context information and
   translation rules.
.. The G-stage and/or VS-stage page table entries to translate the IOVA.
...
.. The page-based memory types (PBMT), if Svpbmt is supported, obtained from the
   ...
   memory type as resolved between the G-stage and VS-stage page table entries.
...
* VS-stage and G-stage virtual-memory system as specified by the RISC-V
  Privileged specification cite:[PRIV] to allow software flexibility to either
  use a common page table for the CPU MMU as well as the IOMMU or to use a
