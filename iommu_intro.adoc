...
PCIe port that maps a portion of a heirarchy through an associated virtual 
...
interfacind devices (IO Devices A and B) to the system fabric without the use
of a Root Port.
...
inbound transactions. The data associaetd with the inbound transactions is not 
...
* Host interface: it is a slave interface to the IOMMU for the harts to access 
  its MMIO registers and perform global configuration and/or maintenance operations.
...
.. The hardware identities associated with transaction - the `device_id` and 
   ...
   retreive the context information to perform the requested address translations.
...
.. The G-state and/or S/VS page table entriess to translate the IOVA
...
* Device Translation Completion interface: it is a master interface which 
  ...
  address translations. The completions interface may provide informations 
  such as:
.. Status of the request to indicate if the request completed successfully or
   a fault occurred.
...
.. The page-based memory types (PBMT), if Svpbmt is supported, obtained from the
   VS/S and/or G-stage page tables. When two-stage addresss translation is
...
physical memory access privileges. As the IOMMU itself is a bus master for its 
