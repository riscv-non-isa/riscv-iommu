...
by any of the harts in the implementation.
...
|0    |`cqen`  |RW        | The command-queue-enable bit enables the command-
                            queue when set to 1. 
                                                                              +
                            Changing `cqen` from 0 to 1 set the `cqh`, and the
                            `cqcsr` bits `cmd_ill`,`cmd_to`, `cqmf`, `fence_w_ip`
                            to 0. The command-queue may take some time to be
                            active following setting the `cqen` to 1. During
                            this delay the `busy` bit is 1. When the command
                            queue is active, the `cqon` bit reads 1.  +
                                                                              +
                            Changing `cqen` from 0 to 1 resets the `cqh` to 0.
                            The command-queue may take some time to be active
                            following setting the `cqen` to 1.  During this 
                            delay the `busy` bit is 1. When the command queue
                            is active, the `cqon` bit reads 1. +
                                                                              +
...
|0     |`fqen`|RW        | The fault-queue enable bit enables the fault-queue 
                           when set to 1. +
                                                                               +
                           Changing `fqen`  from 0 to 1, sets the `fqt`, and the
                           `fqcsr` bits `fqof`, and `fqmf` to 0.
                                                                               +
...
|0     |`pqen`   |RW        | The page-request-enable bit enables the
                              ...
                              and the `pqcsr` bits `pqmf` and `pqof` to 0.
                              The page-request-queue may take some time to be 
                              active following setting the `pqen` to 1. During
                              this delay the `busy` bit is 1. When the 
