...
|===
|Bits   |Field      |Attribute | Description
|13:0   |`eventID`  |WARL      | Indicates the event to count. 
...
|14     |`custom`   |WARL      | When set to 1, `eventID` is custom.

                                 When set to 0, `eventID` is defined in the
                                 <<Event_list>>
...
[[Event_list]]
When `iohpmevt*.custom == 0`, the following table lists the standard events that can be counted:
...
| *eventID* | *Event* | *IDT*
| 0 | Do not count | 
| 1 | Counts the number of Untranslated requests at device interface | IDT==0 filtering available
| 2 | Counts the number of Translated requests at device interface | IDT==0 filtering available
| 3 | Counts the number of ATS Translation requests at device interface | IDT==0 filtering available
| 4 | Counts the number of Device Context cache miss | IDT==0 filtering available 
| 5 | Counts the number of Process Context cache miss | IDT==0 filtering available 
| 6 | Counts the number of TLB miss | IDT==0 or 1 filtering available
| 7 | Counts the number of Device Directory Walks | IDT==0 filtering available 
| 8 | Counts the number of Process Directory Walks | IDT==0 filtering available 
| 9 | Counts the number of S/VS-stage Page Table Walks | IDT==0 or 1 filtering available
| 10 | Counts the number of G-stage Page Table Walks | IDT==0 or 1 filtering available
|===

Some events types may be filtered by IDs. When a event type that does not 
support filtering by IDs is programmed then the associatedcounter does not
increment.

When `iohpmevt*.custom == 1`, events to be counted are custom (implementation defined).
