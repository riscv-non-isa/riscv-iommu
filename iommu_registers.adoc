...
The reset value for the `ddtp.iommu_mode` field must be either `Off` or `Bare`. The
reset value for the `ddtp.busy` field must be 0.

The reset value for the `tr_req_ctl.Go/Busy` field must be 0.
...
an interrupt source in the IOMMU. When an interrupt-pending bit in the register
is set to 1 the IOMMU will not signal another interrupt from that source until
...
Some event types may be filtered by IDs. When an event type that does not
