...
```shell
git clone --recurse-submodules https://github.com/riscv-non-isa/riscv-iommu.git
```

All in one single line:

```shell
git clone --recurse-submodules https://github.com/riscv-non-isa/riscv-iommu.git && cd riscv-iommu && git submodule update --init --recursive
