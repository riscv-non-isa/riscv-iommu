...
substitutes for the targeted virtual interrupt file (as in Figure TBD), or a 
...
the PTE is implementation-defined.
...
The PTE’s `MRIF_ADDR` field provides bits 55:9 of the physical address of a
...
destination MRIF’s address must be zero and are not specified in the PTE.
...
[NOTE]
====
Typically, `NPPN` will be the page address of an IMSIC’s interrupt file in the
...
..  Transaction has a valid `process_id`
..  Transaction type is not supported by the IOMMU in `Bare` mode.
...
the process to identify a incoming 32-bit write made by a device as a MSI write 
and translating the write using the MSI page table is as follows:
...
. If the write has a valid `process_id` (e.g., PASID prefix present), then
  the write is not determined to be a MSI write. Stop this process and instead
  use the regular translation data structures to do the address translation.
. Determine if the write to `A` is an MSI as specified in <<MSI_ID>>.
. If the write is not determined to be an MSI then stop this process and 
...
. If bit 2 of `A` is 1, i.e. the MSI is in big-endian byte order. The IOMMU
  ...
  then stop this process and request the IO bridge to discard the write as an 
  unsupported request.
...
... All DMA writes older than the incoming MSI that was transformed by this 
    process must be visible to all agents before the write to the destination 
    MRIF or to the `NPPN` becomes visible to any agent; unless protocol specific 
    relaxation is allowed (e.g. PCIe relaxed ordering) or is not required.
...  The write to destination MRIF must be visible to all agents in the system 
    before the write to `NPPN` becomes visible to any agent
...
PTEs atomically. The A and/or D bit updates by the IOMMU must follow the rules
specified by the Privileged specification for validity, permission checking, and
atomicity. The PTE update must be visible to all agents i.e., globally visible, 
before a memory access is made using the translated address provided by the 
IOMMU. When the translated address if provided to a device in an ATS Translation
completion, the PTE updates must be globally visible before an explicit memory 
access from the device using the translated address occurs.

If the A bit requires to be set as part of the translation process, the IOMMU
must not cache the contents of the PTE before the PTE update is globally visible.

[NOTE]
====
The A and D bits are never cleared by the implementation. If the supervisor 
software does not rely on accessed and/or dirty bits, e.g. if it does not swap 
memory pages to secondary storage or if the pages are being used to map I/O space, 
...
* If requesting device is a CXL type 1 or type 2 device and the memory
  ...
  setting of this bit is implementation-defined. If the Svpbmt extension is not
   supported then the setting of this bit is implementation-defined.
...
[NOTE]
====
A "Stop Marker" is encoded as a "Page Request" with a PASID prefix but with
the L, W, and R fields set to 1, 0, and 0 respectively.
====

For IOMMU generated "Page Request Group Response" messages that have status 
Invalid Request, the PRG-response-PASID-required (`PRPR`) bit when set to 1 
indicates that the IOMMU response message should include a PASID prefix if 
the associated "Page Request" had a PASID prefix. 

For IOMMU generated "Page Request Group Response" with response code set to 
Response Failure, if the "Page Request" had a PASID prefix then response is 
generated with a PASID prefix.
