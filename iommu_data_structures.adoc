...
stage may be effectivly disabled by configuring the corresponding mode to be
`Bare` and thereby effectively disabling any address translation or protection at
that stage. If the VS-stage mode is configured as `Bare` then the IOVA is equal
to a GPA and there is no further VS-stage memory protection. If the G-stage mode
is configured as `Bare` then the guest physical addressess are equal to
supervisor physical addresses and there is no further G-stage memory protection
...
only the VS-stage page table may be used and the the G-stage may be configured
to be in `Bare` mode.
...
The VS-stage page table formats are as defined by the Privileged
specification.

The `DC.tc.SXL` field controls the supported paged virtual-memory schemes.

The `iovsatp` `MODE` identifies the paged virtual-memory schemes and
...
translation, as controlled by the `iohgatp`, into a supervisor physical address.
Translating addresses of root PDT root through G-stage page tables, allows the
...
When G-stage address translation scheme is not `Bare`, the `PPN`
