...
. `capabilities.BE` is 0 and `fctl.BE != DC.tc.SBE`
...
the endianness of as determined by `fctl.BE` or by `DC.ta.SBE` as follows:
...
| Process directory Table    | `fctl.SBE`
| S/VS-stage page table      | `fctl.SBE`
|===
...
identical `GSCID` and `PSCID` in two DC but with different `SBE` may lead to
the IOMMU interpreting a S/VS-stage PTE as big-endian form or little-endian. These
