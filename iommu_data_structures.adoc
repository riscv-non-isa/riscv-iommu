...
If `GADE` is 1, the IOMMU updates A and D bits in G-stage PTEs atomically else
the IOMMU causes a guest-page-fault.

If `SADE` is 1, the IOMMU updates A and D bits in S/VS-stage PTEs atomically else
the IOMMU causes a page-fault.
