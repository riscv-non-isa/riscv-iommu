...
is 0 then the IOMMU causes a guest-page-fault. If `fctrl.ADFD` is 1, the IOMMU
...
* If the S/VS-stage page table is not `Bare` then a page fault occurs if the
  `IOVA` has bits beyond bit 31 set to 1.
* If the G-stage page table is not `Bare`, then a guest page fault occurs if
  the GPA has bits beyond bit 33 set to 1.
...
| 0    | `Bare`   | No translation or protection. Recognition of accesses to
                    a virtual interrup file using MSI address mask and
...
physical address space of the relevant VM. An incoming memory access made by a device is
recognized as an access to a virtual interrupt file if the destination guest
physical page matches the supplied address pattern in all bit positions that are
zeros in the supplied address mask. In detail, a memory access to guest physical address
`A` is recognized as an access to a virtual interrupt file's memory-mapped page if:
...
. `fctl.SXL` is 1 and `DC.tc.SXL` is 0
...
[[MSI_PT]]
=== MSI page tables
Whenever an IOMMU determines that a memory access is to a virtual interrupt file
using the method specified in <<MSI_ID>>, the access is translated or converted by
...
conversion of accesses made to a particular 4-KiB guest physical page that a
...
address of the incoming memory access by the formula of the <<MSI_ID>>. Each
...
If `V = 1` and the custom-use bit `C = 0`, then bits 2:1 of the first doubleword
is field `M` (Mode).  If `M = 3`, the MSI PTE specifies pass-through
mode for accesses to the page, and if `M = 1`, it specifies MRIF mode. The
values of 0 and 2 for `M` are reserved. The interpretation of an MSI PTE for
each of the two defined modes is detailed further in the next two subsections.

==== MSI PTE, pass-through mode
When an MSI PTE has fields `V = 1`, `C = 0`, and `M = 3` (pass-through mode), the
...
An incoming memory access within the page covered by the MSI PTE is translated by
replacing the write’s original address bits 12 and above (the guest physical page
number) with field `PPN` (Physical Page Number) from the PTE, while retaining the
original address bits 11:0 (the page offset). This translated address is either
zero-extended or clipped at the upper end as needed to make it the width of a
real physical address for the machine.

An MSI PTE in pass-through mode allows a hypervisor to route an MSI write intended
...
For RV64, the first doubleword of an MSI PTE in pass-through mode has the
same encoding as a regular RISC-V leaf PTE for Sv39, Sv48, Sv39x4, or
Sv48x4 page-based address translation, with PTE fields D, A, G, U, and X
all zeros and W = R = 1. Hence, the MSI PTE’s first doubleword appears the same
as a regular PTE that grants read and write permissions (R = W = 1) but not
execute permission (X = 0). This same-encoded regular PTE would
...
`V = 1`, `C = 0`, and `M = 1` (MRIF mode), the PTE’s complete format is:
...
MRIF-mode PTE are handled by the IOMMU instead of being passed through to the
memory system. Read access are treated as Unsupported Requests.  A write access
is either interpreted as an MSI, resulting in an update of the destination MRIF,
or is discarded.
...
pass-through mode (`M = 3`), they might not cache PTEs configured in MRIF mode
(`M = 1`). Two reasons together justify not caching MSI PTEs in MRIF mode: First,
...
. If MSI address translations using MSI page tables is enabled
  ...
  in <<MSI_TRANS>> is invoked. If the GPA `A` is determined to be not an MSI
  then the process continues at step 20. If a fault is detected by the MSI
  address translation process then stop and report the fault else the process
...
the process to identify a incoming `IOVA` from a device as a MSI address and
translating the address using the MSI page table is as follows:
...
.. If the memory access is a read then the IOMMU may either return zero as the
   read value or may abort the transaction as an Unsupported Request. These
   are the only expected behaviors and the process continues at step 15.
.. If the write access is for a data size other than 32-bits or if the address
   is not 32-bit aligned then discard the write access and go to step 15.
...
.. Discard the incoming write access.
