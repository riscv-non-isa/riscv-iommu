...
. If request is a Translated request and `DC.tc.T2GPA` is 1 then the IOVA is a 
  GPA. Go to step 15 with following page table information:
...
. If there is no `process_id` associated with the transaction then go to step 15
...
.. `capabilities.Sv32x4` is 0 and `DC.iohgatp.MODE` `Sv32x4`
.. `capabilities.Sv39x4` is 0 and `DC.iohgatp.MODE` `Sv39x4`
.. `capabilities.Sv48x4` is 0 and `DC.iohgatp.MODE` `Sv48x4`
.. `capabilities.Sv57x4` is 0 and `DC.iohgatp.MODE` `Sv57x4`
.. `iohgatp.PPN` is not aligned to 16 KiB
...
. If `pdte` access detects a data corruption (a.k.a. poisoned data), then 
  stop and report "PDT data corruption" (cause = 269). This fault is detected
  if the IOMMU supports the RAS capability (`capabilities.RAS == 1`).
...
. Let `PC` be value of 16-bytes at address `a + PDI[0] x 16`. If accessing `PC`
  ...
  (cause = 269). This fault is detected if the IOMMU supports the RAS capability
  (`capabilities.RAS == 1`).
...
.. `capabilities.Sv32` is 0 and `PC.fsc.iosatp.MODE` is `Sv32`
.. `capabilities.Sv39` is 0 and `PC.fsc.iosatp.MODE` is `Sv39`
.. `capabilities.Sv48` is 0 and `PC.fsc.iosatp.MODE` is `Sv48`
.. `capabilities.Sv57` is 0 and `PC.fsc.iosatp.MODE` is `Sv57`
...
. If `msipte` access detects a data corruption (a.k.a. poisoned data), then 
  stop and report "MSI PT data corruption" (cause = 270). This fault 
  is detected if the IOMMU supports the RAS capability 
  (`capabilities.RAS == 1`).
...
.. If the MRIF access detects a data corruption (a.k.a poisoned data), then 
   stop and report "MSI MRIF data corruption" (cause = 271). This fault 
   is detected if the IOMMU supports the RAS capability 
   (`capabilities.RAS == 1`).
