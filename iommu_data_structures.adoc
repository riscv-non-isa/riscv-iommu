...
[NOTE]
====
If the `dc` cannot be located to process a "Page Request" then the the IOMMU 
cannot determine the `EN_PRI` and `PRPR` controls. The IOMMU generates a 
"Page Request Group Response" with response code set to Response Failure. If
the "Page Request" had a PASID prefix then reponse is generated with a PASID
prefix.
====
...
[[ATS_FAULTS]]
=== PCIe ATS translation request fault handling
