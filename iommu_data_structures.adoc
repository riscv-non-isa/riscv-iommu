...
If `DC.tc.PDTV` is 0, the `fsc` field in `DC` holds the `iosatp` that provides
...
When `DC.tc.PDTV` is 1, the `fsc` field holds the process-directory table pointer
...
as controlled by the `DC.iohgatp`, into a supervisor physical address. A guest OS
...
[NOTE]
====
The `PC.ta.PSCID` field of `PC` identifies an address space. If an identical
