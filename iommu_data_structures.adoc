...
===== First-Stage context (`fsc`)
The `PC.fsc` field holds the `iosatp` (<<IOSATP_FORMAT>>) that provide the
controls for first-stage address translation and protection. 

The `iosatp.MODE` is used to determine the first-stage paged virtual-memory
scheme and its encodings are as defined in <<IOSATP_MODE_ENC>>. The `DC.tc.SXL`
field controls the supported paged virtual-memory schemes. When `iosatp.MODE` is
not `Bare`, the `iosatp.PPN` field holds the PPN of the root page of a
first-stage page table.

When second-stage address translation is not Bare, the `iosatp.PPN` field holds
