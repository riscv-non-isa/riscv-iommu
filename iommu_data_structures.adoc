...
redirecting MSIs to virtual machines is specified by the RISC-V AIA specification
cite:[RISCV-AIA].
...
The G-stage page table formats are as defined by the RISC-V Privileged
specification cite:[PRIV]. The `fctl.GXL` field controls the supported
address-translation schemes for guest physical addresses as defined in
<<IOHGATP_MODE_ENC>>.
...
defined by the RISC-V AIA specification cite:[RISCV-AIA].
...
. If a G-stage page table is not active in the device-context then use the
  ...
  RISC-V Privileged specification cite:[PRIV]. If a fault is detecting by the
  single stage address translation process then stop and report the fault.
...
Interrupt Architecture cite:[RISCV-AIA], MSIs are sent to virtual harts within
the VM by writing to the memory-mapped registers of the interrupt files of
virtual IMSICs. Each of these virtual interrupt files occupies a separate 4-KiB
page in the VMs guest physical address space, the same as real interrupt files
do in a real machines physical address space. A write to a guest physical
address can thus be recognized as an MSI to a virtual hart if the write is to a
...
. If `msipte.M == 3` the PTE is translate-R/W mode mode PTE and the translation
...
[NOTE]
====
In MRIF mode, the RISC-V AIA specification cite:[RISCV-AIA] defines the operation to
...
. The A and/or D bit updates by the IOMMU must follow the rules specified by the
  RISC-V Privileged specification cite:[PRIV] for validity, permission checking,
  and atomicity.
...
in the RISC-V Privileged specification cite:[PRIV] cause the IOVA translation
process to stop and report the detected fault.
...
No faults are logged in the fault queue for PCIe ATS "Page Request" messages
cite:[PCI] for following conditions:
