...
The IOMMU employs a two-stage address translation process to perform address
translation and memory protection. The first stage (VS-stage) is used to convert
an IOVA into a guest physical address. The guest physical address is then
converted in the second stage (G-stage) into a supervisor physical address.
Although there is no option to disable two-stage address translation, either
stage may be effectivly disabled by configuring the corresponding address
translation scheme to be `Bare` and thereby effectively disabling any address
translation or protection by that stage. If the VS-stage scheme is configured as
`Bare` then the IOVA is equal to a GPA and there is no further VS-stage address
translation or memory protection. If the G-stage scheme is configured as `Bare`
then the guest physical addressess are equal to supervisor physical addresses
and there is no further G-stage address translation or memory protection

The address space used by a device may require G-stage address translation and
protection when the control of the device is passed through to a Guest OS. When
the control of the device is retained by the hypervisor or Host OS itself then
the G-stage scheme may be configured to be `Bare`.

When G-stage address translation scheme is not `Bare` the `DC` holds the PPN of
the root G-stage page table; a guest-soft-context-ID (`GSCID`), which
facilitates invalidation of cached address translations on a per-virtual-machine
basis; and the G-stage address translation scheme.
...
When a PDT is active, the controls for VS-stage address translation are held in
the (`PC`).

When a PDT is not active, the controls for VS-stage address translation are held
in the `DC` itself.

The VS-stage address translation controls include the PPN of the root
VS-stage page table; a process-soft-context-ID (`PSCID`), which facilitates
invalidation of cached address translations on a per-address-space basis; and
the VS-stage address translation scheme.
...
The `fsc` field of `DC` holds the context for first-stage translation
(VS-stage). If the `PDTV` bit is 1, the field holds the PPN of the root page
of PDT.  If the `PDTV` bit is 0, the `fsc` field holds the PPN of the root page
of a VS-stage page table (`iovsatp`).
...
If `SADE` is 1, the IOMMU updates A and D bits in VS-stage PTEs atomically. If
...
If `SBE` is 0, implicit memory accesses to PDT entries and VS-stage PTEs are
...
* If the VS-stage mode is not `Bare` then a page fault corresponding to
  the original access type occurs if the `IOVA` has bits beyond bit 31 set to 1.
* If the G-stage mode is not `Bare`, then a guest page fault corresponding
  to the original access type occurs if the incoming GPA has bits beyond bit 33
  set to 1.
...
`GSCID` is configured in two `DC` when the G-stage page-table referenced by the
two `DC` are not identical then it is unpredictable whether the IOMMU uses the
...
address-space ID if `PDTV` is 0 and the `iovsatp` `MODE` field is not
`Bare`. When `PDTV` is 1, the `PSCID` field in `ta` is ignored.

===== First-Stage context (`fsc`)
If `PDTV` is 0, the `fsc` field in `DC` holds the `iovsatp` that provide the
controls for VS-stage address translation and protection.

.IO Virtual Supervisor address translation and prot. (`iovsatp`) field (when `PDTV` is 0)
[wavedrom, , ]
....
{reg: [
  {bits: 44, name: 'PPN'},
...
The VS-stage page table formats are as defined by the Privileged specification.

The `DC.tc.SXL` field controls the supported paged virtual-memory schemes.

The `iovsatp.MODE` identifies the paged virtual-memory schemes and is encoded
as follows:

[[IOSATP_MODE_ENC]]
.Encodings of `iovsatp.MODE` field
[width=75%]
[%header, cols="3,3,20"]
|===
3+^|               `DC.tc.SXL=0`
...
`process_id`, the PDT is used to determine the VS-stage page table and
...
When G-stage address translation mode is not `Bare`, the `PPN` field holds a
...
address. Translating addresses of PDT using a G-stage page table, allows the PDT
to be held in memory allocated by the guest OS and allows the guest OS to
directly edit the PDT to associate a virtual-address space identified by a
VS-stage page table with a `process_id`.
...
|===
|Value | Name     | Description
| 0    | `Bare`   | No VS-stage address translation or protection.
...
.IO Virtual Supervisor address translation and protection (`iovsatp`) field (when `PDTV` is 0)
[wavedrom, , ]
....
{reg: [
  {bits: 44, name: 'PPN'},
...
A valid (`V==1`) leaf PDT entry holds the PPN of the root page of a VS-stage
page table and the `MODE` used to determine the VS-stage address translation
...
space ID for the process identified by the VS-stage page table.

When G-stage address translation scheme is not `Bare`, the `PPN` field holds a
guest PPN of the root of a VS-stage page table. Addresses of the VS-stage page
table entries are then converted by guest physical address translation process,
as controlled by the `iohgatp`, into a supervisor physical address. A guest OS
may thus directly edit the VS-stage page table to limit access by the device to
a subset of its memory and specify permissions for the device accesses.

[NOTE]
====
The `PSCID` field of `PC` identified an address space. If an identical
...
. If request is a Translated request and `DC.tc.T2GPA` is 1 then the IOVA is a
  GPA. Go to step 18 with following page table information:
...
. If `DC.tc.PDTV` is set to 0 then go to step 17 with the following page table
  information:
.. Let `iovsatp.MODE` be the value in the `DC.fsc.MODE` field
.. Let `iovsatp.PPN` be the value in the `DC.fsc.PPN` field
...
.. Let `iovsatp.MODE` be the value in the `PC.fsc.MODE` field
.. Let `iovsatp.PPN` be the value in the `PC.fsc.PPN` field
.. Let `PSCID` be the value in the `PC.ta.PSCID` field
.. Let `iohgatp` be the value in the `DC.iohgatp` field
. Use the process specified in Section "Two-Stage Address Translation" of the RISC-V
...
. If MSI address translations using MSI page tables is enabled
  ...
  address of a virtual interrupt file then the process continues at step 19. If
...
. Use the G-stage address translation process specified in Section "Two-Stage Address
...
Privileged specification cite:[PRIV] cause the IOVA translation process to stop and
...
process-directory-table, VS and G-stage translation tables, and/or MSI page
tables. These caches are collectively referred to as the IOMMU Address
Translation Caches (IOATC).

This specification does not allow the caching of VS/G-stage PTEs whose `V`
...
|VS-stage page tables
 (G-stage is not `Bare`)|`GSCID`, `PSCID`, and IOVA | <<IVMA, IOTINVAL.VMA>>
|VS-stage page tables   
 (G-stage is `Bare`)    |`PSCID`, and IOVA          | <<IVMA, IOTINVAL.VMA>>
