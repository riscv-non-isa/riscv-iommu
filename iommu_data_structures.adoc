...
When checking the `U` bit in a G-stage PTE, the privilege requested by the
transaction is ignored and the transaction is treated as not requesting
...
. If the request is a Untranslated or Translated read-for-execute transaction
  then stop this process and instead use the regular translation data structures
...
. The access permissions associated with the translation determined through this
  process are equivalent to that of a regualar RISC-V G-stage PTE with
  `R`=`W`=`U`=1 and `X`=0. Similar to a G-stage PTE, when checking the `U` bit,
  the request is treated as not requesting supervisor privilege.
