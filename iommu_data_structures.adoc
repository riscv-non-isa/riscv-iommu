...
first-stage is not required, then it may be disabled by selecting the
...
protections; the second-stage scheme may be disabled for the device by
programming the second-stage address translation scheme to be `Bare`.

When second-stage address translation is enabled, the `DC` holds the PPN of the
...
The `fsc` field of `DC` holds the context for first-stage translation. If the
`PDTV` bit is 1, the field holds the PPN of the root page of PDT.  If the
`PDTV` bit is 0, the `fsc` field holds the PPN of the root page of a first-stage
page table (`iosatp`).
...
* If the first-stage is enabled, then a page fault corresponding to the original
  access type occurs if the `IOVA` has bits beyond bit 31 set to 1.
* If the second-stage is enabled, then a guest page fault corresponding to the
...
address-space ID if `PDTV` is 0 and the `iosatp` `MODE` field is not `Bare`.
When `PDTV` is 1, the `PSCID` field in `ta` is ignored.

===== First-Stage context (`fsc`)
If `PDTV` is 0, the `fsc` field in `DC` holds the `iosatp` that provide the
controls for first-stage address translation and protection.

.IO Supervisor address translation and prot. (`iosatp`) field (when `PDTV` is 0)
[wavedrom, , ]
....
{reg: [
  {bits: 44, name: 'PPN'},
...
When second-stage address translation is enabled, the `PPN` field holds a
...
.IO Supervisor address translation and protection (`iosatp`) field (when `PDTV` is 0)
[wavedrom, , ]
....
{reg: [
  {bits: 44, name: 'PPN'},
...
space ID for the process identified by the first-stage page table.

When second-stage address translation is enabled, the `PPN` field holds a guest
PPN of the root of a first-stage page table. Addresses of the first-stage page
...
may thus directly edit the first-stage page table to limit access by the device
to a subset of its memory and specify permissions for the device accesses.
...
... The `PSCID` value is not used when first-stage is not enabled.
...
.. Let `iohgatp` be the value in the `DC.iohgatp` field
. If second-stage address translation is not enabled
  (i.e., `DC.iohgatp.MODE = Bare`) then use the single-stage address translation
  process specified in Section "Virtual Address Translation Process" of the
  RISC-V privileged specification cite:[PRIV]. If a fault is detected by the
  single-stage address translation process then stop and report the fault else
  go to step 21.
. If second-stage address translation is enabled
  (i.e., `DC.iohgatp.MODE != Bare`), then use the process specified in Section
  "Two-Stage Address Translation" of the RISC-V Privileged specification
  cite:[PRIV] to determine the GPA accessed by the transaction. If a fault is
  detected by the two-stage address translation process then stop and report the
  fault. If the translation process is completed successfully then let `A` be the
  translated GPA.
. If MSI address translations using MSI page tables is enabled
  (i.e., `DC.msiptp.MODE != Bare`) then the MSI address translation process
  specified in <<MSI_TRANS>> is invoked. If the GPA `A` is not determined to be
  the address of a virtual interrupt file then the process continues at step 20.
...
. If second-stage address translation is enabled then use the
  second-stage address translation process specified in Section "Two-Stage
  Address Translation" of the RISC-V Privileged specification cite:[PRIV] to
  translate the GPA `A` to determine the SPA accessed by the transaction. If a
  fault is detected by the address translation process then stop and report the
  fault.
...
Faults detected during the single/two-stage address translation specified in the
RISC-V Privileged specification cite:[PRIV] cause the IOVA translation process
to stop and report the detected fault.
...
|First-stage page tables
 (second-stage is not
  enabled)              |`GSCID`, `PSCID`, and IOVA | <<IVMA, IOTINVAL.VMA>>
|First-stage page tables   
 (second-stage is
  enabled)              |`PSCID`, and IOVA          | <<IVMA, IOTINVAL.VMA>>
