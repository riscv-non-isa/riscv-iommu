...
. Let `a` be `ddtp.PPN x 4096` and let `i = LEVELS - 1`. When 
...
. If if any bits or encodings that are reserved for future standard use are 
  ...
  (cause = 259).
. Let `i = i - 1` and let `a = ddte.PPN x PAGESIZE`. Go to step 3.
...
See <<FAULT_QUEUE>> for fault/event reporting and fault record formats.
...
. Let `a` be `pdtp.PPN x 4096` and let `i = LEVELS - 1`. When 
...
. If if any bits or encodings that are reserved for future standard use are 
  ...
  (cause = 267).
. Let `i = i - 1` and let `a = pdte.PPN x PAGESIZE`. Go to step 2.
