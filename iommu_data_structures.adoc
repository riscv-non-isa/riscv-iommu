...
If `PDTV` is 0, the `fsc` field in `DC` holds the `iosatp` that provides the
...
.. Let `iohgatp` be the value in the `DC.iohgatp` field
. If second-stage address translation is not enabled, then use the process
  specified in Section "Two-Stage Address Translation" of the RISC-V Privileged
  specification cite:[PRIV]. If a fault is detected by the address translation
  process then stop and report the fault else go to step 21.
. If second-stage address translation is enabled then use the process specified
  in Section "Two-Stage Address Translation" of the RISC-V Privileged
  specification cite:[PRIV] to determine the GPA accessed by the transaction. If
  a fault is detected by the two-stage address translation process then stop and
  report the fault. If the translation process is completed successfully then
  let `A` be the translated GPA.
...
|First-stage page table   
 (when second-stage is
  not enabled)          |`PSCID`, and IOVA          | <<IVMA, IOTINVAL.VMA>>
