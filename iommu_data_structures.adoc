...
Reserved bits, `rsvd`, of the first doubleword must be set to zeros by software.
The second doubleword is ignored by an IOMMU so is free for software to use.
...
Reserved bits, `rsvd`, of the PTE must be set to zeros by software.
...
Field `NPPN` (Notice Physical Page Number) and the two `NID` (Notice Identifier)
fields together specify a destination and value for a notice MSI that is sent
...
• no memory-resident interrupt files (`capabilities.MSI_MRIF = 0`);
• memory-resident interrupt files without atomic update 
  (`capabilities.MSI_MRIF=1` and `capabilities.AMO=0`); or
• memory-resident interrupt files with atomic update.
  (`capabilities.MSI_MRIF=1` and `capabilities.AMO=1`); or
...
.. If the IOMMU supports atomic memory operations 
   ...
   (at address `msipte.MRIF_ADDR[55:9] << 9`), set the interrupt-pending bit 
...
.. If the IOMMU does not support atomic memory operations then, in the 
   destination MRIF (at address `msipte.MRIF_ADDR[55:9] << 9`), set the 
...
.. Zero-extend the 11-bit `msipte.NID` value to 32 bits, and do a 32-bit write
   of this value in little-endian byte order to the address `msipte.NPPN << 12`
   (i.e., physical page number `NPPN`, page offset zero).
