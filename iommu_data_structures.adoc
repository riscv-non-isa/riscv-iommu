...
as defined in <<IOSATP_MODE_ENC>>. The `PPN` field holds the PPN of the root
page of a first-stage page table.

When second-stage address translation is not `Bare`, the PPN is a guest PPN. The
GPA of the root page is then converted by guest physical address translation
...
===== First-Stage context (`fsc`)
The `fsc` field in `PC` holds the `iosatp` (<<IOSATP_FORMAT>>) that provide the
controls for first-stage address translation and protection. 

The `MODE` is used to determine the first-stage paged virtual-memory scheme and
its encodings are as defined in <<IOSATP_MODE_ENC>>. The `DC.tc.SXL` field
controls the supported paged virtual-memory schemes. When `MODE` is not `Bare`,
the `PPN` field holds the PPN of the root page of a first-stage page table.
...
[NOTE]
====
The `PSCID` field of `PC` identifies an address space. If an identical
