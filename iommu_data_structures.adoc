...
The encoding of the `iohgatp` `MODE` field identifies the paged virtual-memory
schemes and its settings are as follows:
...
| 1    | `Sv32x4` | Page-based 34-bit virtual addressing (2-bit extension
                    of Sv32).
| 2-7  | --       | Reserved for standard use.
...
The encoding of the `iosatp`/`iovsatp` `MODE` field identifies the paged
virtual-memory schemes and its settings are as follows:

[[IOSATP_MODE_ENC]]
.Encoding of `iosatp/iovsatp.MODE` field
[width=75%]
[%header, cols="3,3,20"]
|===
|Value | Name     | Description
...
scheme. The `MODE` field encoding are as defined in <<IOSATP_MODE_ENC>>.
