...
.. If any of the following conditions are true then then stop and report
   "Transaction type disallowed" (cause = 260).
... Memory access is a read
... Write access is for a data size other than 32-bits
... Address of the write access is not 32-bit aligned
...
.. Request the IO bridge to discard the incoming write access.
