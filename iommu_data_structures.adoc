...
When second-stage address translation scheme is not `Bare` the `DC` holds the
PPN of the root second-stage page table; a guest-soft-context-ID (`GSCID`), which
...
* If the first-stage mode is not `Bare` then a page fault corresponding to
  the original access type occurs if the `IOVA` has bits beyond bit 31 set to 1.
* If the second-stage mode is not `Bare`, then a guest page fault corresponding
...
When second-stage address translation mode is not `Bare`, the `PPN` field holds a
...
When second-stage address translation scheme is not `Bare`, the `PPN` field
holds a guest PPN of the root of a first-stage page table. Addresses of the
first-stage page table entries are then converted by guest physical address
translation process, as controlled by the `iohgatp`, into a supervisor physical
address. A guest OS may thus directly edit the first-stage page table to limit
