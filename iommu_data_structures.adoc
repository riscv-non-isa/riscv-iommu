...
64-bit doublewords in extended-format. The IOMMU may read the `DC` fields in any order.
...
When two-stage address translation is active, the `VSBE` bit controls whether
implicit memory accesses to PDT entries and VS-stage PTEs are little-endian
(`VSBE=0`) or big-endian (`VSBE=1`). The supported values of `VSBE` are the same
as that of the `fctl.SBE` field.
...
. `capabilities.BE` is 1 and endianness configured in `DC.tc.VSBE` is
  not supported.
...
The `PC` is interpreted as two 64-bit doublewords. The IOMMU may read the `PC`
fields in any order.
...
endianness as determined by `fctl.SBE` (<<FCTRL>>).
...
The byte order of each double word in memory, little-endian or big-endian, is
the endianness of as determined by`fctl.SBE` or by `DC.ta.VSBE` as follows:
...
| Process directory Table +
  (When single-stage address
   translation is active)    | `fctl.SBE`
| S-stage page table         | `fctl.SBE`
| G-stage page table         | `fctl.SBE`
| MSI page table             | `fctl.SBE`
| Process directory Table +
  (When two-stage address
   translation is active)    | `fctl.VSBE`
| VS-stage page table        | `fctl.VSBE`
|===
...
identical `GSCID` and `PSCID` in two DC but with different `VSBE` may lead to
the IOMMU interpreting a VS-stage PTE as big-endian form or little-endian. These
are the only behaviors are expected.
====
