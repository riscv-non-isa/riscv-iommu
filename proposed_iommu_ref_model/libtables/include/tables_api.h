...
uint8_t add_process_context(device_context_t *DC, process_context_t *PC, uint32_t process_id);
...
uint8_t add_vs_stage_pte(iosatp_t satp, uint64_t va, pte_t pte, uint8_t add_level, iohgatp_t iohgatp);
...
extern uint64_t get_free_gppn(uint16_t num_gppn, uint8_t map, iohgatp_t iohgatp);
