...
determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[]).
...
extension xref:iommu_extensions.adoc#NLINV[]. The definition of the `S` bit is provided by the address
range invalidation extension xref:iommu_extensions.adoc#ARINV[].
...
specified to have a timeout, then the `cmd_to` bit in `cqcsr` xref:iommu_registers.adoc#CSR[] is set to
...
from the command queue to be generated (by setting `cqcsr.fence_w_ip` - xref:iommu_registers.adoc#CSR[])
...
write encounters a memory fault, the `cmd_mf` bit in `cqcsr` xref:iommu_registers.adoc#CSR[] is set to
...
supported by the IOMMU (see xref:iommu_registers.adoc#CAP[]).
...
as determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[]).
...
0 and 275 that are not specified in xref:iommu_in_memory_queues.adoc#FAULT_CAUSE[] are reserved for future
standard use.
...
Response” message as specified in xref:iommu_data_structures.adoc#ATS_PRI[].
...
endianness as determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[]).
endianness as determined by `fctl.BE` (xref:iommu_registers.adoc#FCTRL[]).
