...
|1   |1   |1     | Invalidates cached information from leaf 
                   ...
                   operand. Global mappings may not be invalidated.
|===

`IOTINVAL.GVMA` guarantees that previous stores made to the G-stage page 
...
addresses. If G-stage page table used for these translations are modified, software
must send the appropriate IOTINVAL command and IODIR command to the Command-queue.
Indeed, some implementations may choose to cache the translated pointers in the 
IOMMU directory caches. IOTINVAL has no effect on the IOMMU directory caches.
====
