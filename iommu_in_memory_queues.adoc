...
* A page-request queue (`PQ`) used by IOMMU to report “Page Request” messages 
  ...
  PCIe defined Page Request Interface cite:[PCI].
...
message to the PCIe cite:[PCI] device function identified by `RID`. An 
...
[NOTE]
====
A Hierarchy is a PCI Express I/O interconnect topology cite:[PCI], wherein the
