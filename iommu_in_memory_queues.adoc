...
then the command-queue sets the `cqcsr.cmd_ill` bit and stops processing
commands from the command-queue. To re-enable command processing software should
clear this bit by writing 1. A command is determined to be illegal if it uses a
uses a reserved encoding or if a reserved bit is set to 1. A command is
unsupported if it is defined but not implemented as determined by the IOMMU
...
`ADDR[63:2] * 4` as a 4-byte store when the command completes.
...
operand must be 1 for `IODIR.INVAL_PDT` else the command is illegal.
...
operand and all associated PDT entries. The `PID` operand is reserved for
`IODIR.INVAL_DDT`.
